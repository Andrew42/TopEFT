...
if [ -d "$GENPRODPATH" ]; then
    ...
    cd ${GENPRODPATH}
    # copy relevant code  
    for FILE in addons patches Utilities runcmsgrid_LO.sh gridpack_generation.sh submit_madpack_ttbareft.sh configure_gridpack.py transfer_gridpacks.py transfer_diagrams.py helper_tools.py ScanType.py BatchType.py DegreeOfFreedom.py submit_cmsconnect_gridpack_generation.sh ; do 
